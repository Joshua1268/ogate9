...
import { rechercherPaiements } from "../../services/paiements/PaiementRequest";
import { rechercherListeTypeServiceparparam } from "../../services/typeServices/TypeServiceRequest";

const liste = [
  {
    transaction: "Kouassi Georgette",
    adresse: "Rivera palmeraie",
    typeService: "Type de service 1",
    menagere: "Kouassi Georgette",
    entreprise: "Skan Technologie",
    montant: "2700",
    date: "09-08-2023, 09:27",
    email: "exemple@gmail.com",
    telephone: "+2250778183123",
    statut: "ATTENTE",
    enabled: true,
  },
  {
...
const HistoriquePaiement = () => {
  ...
  const [transactionListe, setTransactionListe] = useState([]);
  ...
  const TableElement = ({ paiement }) => {
    return (
      <tr>
        <td className="px-4 text-sm text-stone-500 font-normal border-b border-r">
          {paiement.client ? paiement.client : "-"}
        </td>
        <td className="px-4 text-sm text-stone-500 font-normal border-b border-r">
          {paiement.typeService ? paiement.typeService : "-"}
        </td>
        <td className="px-4 text-sm text-stone-500 font-normal border-b border-r">
          {paiement.numero ? paiement.numero : "-"}
        </td>
        <td className="px-4 text-sm text-stone-500 font-normal border-b border-r">
          {paiement.operateur.designation
            ? paiement.operateur.designation
            : "-"}
        </td>
        <td className="px-4 text-sm text-stone-500 font-normal border-b border-r">
          {paiement.montant ? `${paiement.montant} F CFA` : "-"}
        </td>
        <td className="px-4 text-sm text-stone-500 font-normal border-b border-r">
          {paiement.datePaiement ? paiement.datePaiement : "-"}
        </td>
  ...
  //   };
  //   rechercherPaiements(pageQuery.page, pageQuery.size, data)
  //     .then((res) => {
  //       setLoadingData(false);
  //       setTransactionListe(res.data.donnee.paiements);
  //       setPageQuery((prev) => ({
  //         ...prev,
  ...
  const searchPaiement = () => {
    ...
    rechercherPaiements(pageQuery.page - 1, pageQuery.size, data)
      .then((res) => {
  ...
  const handlePaginationChange = (event, value) => {
    ...
    if (searchTerm.length !== 0) {
      ...
      rechercherPaiements(value - 1, pageQuery.size, data)
        .then((res) => {
          setLoadingData(false);
          setTransactionListe(res.data.donnee.paiements);
          setPageQuery((prev) => ({
            ...
            <table className="custom-table table table-sm w-full">
              <thead>
                <tr className="bg-primary/10 h-12">
                  ...
                  <th className="text-sm text-black">Type service</th>
                  ...
                  <th className="text-sm text-black">Date paiement</th>
                  <th className="text-sm text-black">Action</th>
                </tr>
              </thead>
              <tbody>
                {!loadingData && transactionListe.length > 0 ? (
                  transactionListe.map((item) => (
                    <TableElement key={item.id} paiement={item} />
                  ))
                ) : !loadingData && transactionListe.length === 0 ? (
                  <tr>
                    <td colSpan="7">
